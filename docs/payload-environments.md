...
Audit events are append-only from authenticated internal hooks. REST/GraphQL/user Local API create, update, and delete are denied. Events record actor, entity, event type, UTC occurrence time, request ID/IP when available, and sanitized before/after values. IP is accepted only from the framework-provided `req.ip`; client-supplied `X-Forwarded-For` is intentionally ignored. Passwords, hashes, salts, sessions, cookies, authorization values, API keys, secrets, and tokens are redacted before persistence. Strings are limited to 2,048 characters, arrays to 100 entries, object values to 100 keys, and nesting to 8 levels with explicit serializable truncation markers. See `docs/audit-governance.md` for trusted-proxy, retention, backup/restore, privileged-access, and production release requirements.
