...
test("needsTranslation ignores measurements, units, and model codes but keeps real words", async () => {
  ...
  assert.equal(needsTranslation("Mô-men xoắn tối đa (Nm/ft-lb)"), false);
  assert.equal(needsTranslation("Mức ồn (dBA)"), false);
  assert.equal(needsTranslation("Khối lượng (lbs/kgs)"), false);
  assert.equal(needsTranslation("Lưu lượng: 5.5L/min"), false);
  assert.equal(needsTranslation("Tốc độ: 15000-22000/min"), false);
  assert.equal(needsTranslation("Lưu lượng: 3.0m³/min"), false);
  assert.equal(needsTranslation("min speed"), true);
  assert.equal(needsTranslation("with/min"), true);
  assert.equal(needsTranslation("3-4-5-6mm"), false);
  assert.equal(needsTranslation("3-4-5-Green"), true);
  assert.equal(needsTranslation("4-12x2mm"), false);
  assert.equal(needsTranslation("4-12x2Green"), true);
  assert.equal(needsTranslation("22-10AWG"), false);
  assert.equal(needsTranslation("AAA"), false);
  assert.equal(needsTranslation("BSPT"), false);
  assert.equal(needsTranslation("AWS A5.18"), false);
  assert.equal(needsTranslation("Độ sáng: 12 lumen"), false);
  assert.equal(needsTranslation("Kích thước (with case)"), true);
